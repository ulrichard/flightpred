...
module grib_api
  ...
  subroutine grib_pbopen ( ifile, filename, mode, status )
        ...
        if (present(status)) then
           ...
           call grib_check(iret,'grib_pbopen','('//filename//')')
        endif
  end subroutine grib_pbopen
  ...
  subroutine grib_pbread ( ifile, buffer, nbytes, status )
        ...
        if (present(status)) then
           ...
           call grib_check(iret,'grib_pbread','')
        endif
  end subroutine grib_pbread
  ...
  !>
  !> @param gribid      id of the grib loaded in memory
  !> @param message     integer(4) array containing the coded message
  !> @param status      GRIB_SUCCESS if OK, integer value on error
  ...
  end subroutine grib_new_from_message
  ...
  !>
  !>
  !> \b Examples: \ref sample.f90 "sample.f90"
  !>
  !> @param gribid       id of the grib loaded in memory
  ...
  subroutine grib_keys_iterator_new ( gribid, iterid, namespace, status )
      integer(kind=kindOfInt),          intent(in)   :: gribid
      integer(kind=kindOfInt),          intent(out)  :: iterid
      character(LEN=*), intent(in)                   :: namespace
      integer(kind=kindOfInt),optional, intent(out)  :: status
      integer(kind=kindOfInt)                        :: iret
