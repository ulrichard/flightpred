...
*/
  ...
  if(boustrophedonic)
    reverse_rows      (sec_val,  n_vals,  nap);
...
*/
  ...
  buf = NULL;
  buf = grib_context_malloc_clear(a->parent->h->context,buff_len);
...
static long value_count(grib_accessor* a){
  ...
  if(grib_get_long_internal(a->parent->h,self->two_ordr_spd, &two_ordr_spd) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->plus1_spd, &plus1_spd) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->width_lengths, &nbits_per_lengths) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->offsetsection, &offsetsection) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->octet_start_group, &octet_start_group) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->p1, &p1) != GRIB_SUCCESS)
    return 0;
