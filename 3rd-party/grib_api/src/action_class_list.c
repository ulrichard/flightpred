...
#if 0
static int create_accessor_buffer(grib_section* p, grib_action* act)
{
	grib_action_list* a = ( grib_action_list*)act;
	...
	int ret = 0;
	long val = 0;
	grib_expression_evaluate_long(p->h,a->expression,&val);
	...
	while(val--){
		next = la;
		while(next){
			ret =   grib_create_accessor(gs, next,NULL);
