...
/*
  ...
  MEMBERS = const char* process_unit
  MEMBERS = const char* process_time_range
...
static void init(grib_accessor* a,const long l, grib_arguments* c)
{
  ...
  self->process_unit  = grib_arguments_get_name(a->parent->h,c,n++);
  self->process_time_range  = grib_arguments_get_name(a->parent->h,c,n++);
}
...
static int unpack_long(grib_accessor* a, long* val, size_t *len)
{
  ...
  long unit;
  long process_unit;
  long process_time_range;
  long process_time_range_sec=0;
  ...
  if((err = grib_get_long_internal(h,self->process_unit,&process_unit))) return err;
  if((err = grib_get_long_internal(h,self->process_time_range,
      &process_time_range))) return err;

  if (process_unit!=unit) {
    process_time_range_sec=process_time_range*u2s[process_unit];
    if (process_time_range_sec%u2s[unit]!=0) {
      grib_context_log(h->context,GRIB_LOG_ERROR,"unable to convert endStep in stepUnits");
      return GRIB_WRONG_STEP_UNIT;
    }
    process_time_range = process_time_range_sec / u2s[unit];
  }

  *val = start_step + process_time_range;
...
static int pack_long(grib_accessor* a, const long* val, size_t *len)
{
  ...
  /*point in time */
  if (self->year == NULL) {
    err = grib_set_long_internal(h,self->start_step,start_step);
    return err;
  }
  ...
  process_time_range  = *val-start_step;

  if (process_time_range<0){
  ...
  dend+=(((double)(*val))*u2s[unit])/u2s[2];
  ...
  if((err = grib_set_long_internal(a->parent->h,self->process_unit,
      unit))) return err;
  if((err = grib_set_long_internal(a->parent->h,self->process_time_range,
      process_time_range))) return err;
