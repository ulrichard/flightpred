...
static void check_sections(grib_section *s,grib_handle* h)
{
  ...
  {
    Assert(a->parent->h == h);
    check_sections(grib_get_sub_section(a),h);
    a = a->next;
  }
}
                         ...
                         grib_accessor* changed)
{
  ...
  old_section = grib_get_sub_section(notified);
  Assert(old_section);
  ...
  grib_swap_sections(old_section,
      grib_get_sub_section(tmp_handle->root->block->first));

  /* check_sections(old_section,h); */
