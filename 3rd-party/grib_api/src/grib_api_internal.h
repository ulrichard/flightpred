...
#define MAX_NUM_ACCESSORS 1000
#define MAX_NUM_CONCEPTS 1000
...
struct grib_action
{
	...
	int                     priority;     /** higher priority keys must be set before **/
};
...
#define GRIB_ACCESSOR_FLAG_REQUIRED         (1<<12)
...
struct grib_handle
{
    ...
    int  use_trie;
    int  trie_invalid;
    grib_accessor* accessors[MAX_NUM_ACCESSORS];
...
typedef struct grib_string grib_string;
struct grib_string {
  char* string;
  grib_string* next;
};
...
struct grib_context
{
    ...
    grib_string*                    grib_definition_files_dir;
