    ...
    if ( (fabs(lat_first-lats[0]) >= d ) ||
      ...
      *val=0;
      for (j=0;j<nj;j++) {
        ...
        printf("--  %d ",j);
#endif
        grib_gaussian_reduced_sub_row(pl[j],lon_first,lon_last,&row_count,&ilon_first,&ilon_last);
        lon_first_row=((ilon_first-1)*360.0)/pl[j];
        lon_last_row=((ilon_last-1)*360.0)/pl[j];
        *val+=row_count;
#if EFDEBUG
