...
static grib_accessor* search(grib_section* s,const char* name,const char* name_space)
{
	...
	while(a)
	{
		grib_section* sub = grib_get_sub_section(a);
...
static void rebuild_itrie (grib_handle* h,grib_section* s)
{
	grib_accessor* a = s ? s->block->first : NULL;
	...
	while(a)
	{
		...
		while(i<MAX_ACCESSOR_NAMES && ((p = a->all_names[i]) != NULL)) {
			if(*p != '_')   {
				id=grib_itrie_get_id(a->parent->h->context->keys,p);
...
static grib_accessor* search_and_cache(grib_handle* h, const char* name,const char *namespace)
{
	if(h->use_trie)
	{
		...
		{
			int i=0;
			for (i=0;i<h->context->keys_count;i++) 
				h->accessors[i]=NULL;

			if (h->root) 
				rebuild_itrie(h,h->root);

			h->trie_invalid=0;
			id = grib_itrie_get_id(h->context->keys,name);
...
grib_accessor* grib_find_accessor_fast(grib_handle* h, const char* name)
{
	...
	if ( *p == '.' ) {
		...
		a = h->accessors[grib_itrie_get_id(h->context->keys,name)];
