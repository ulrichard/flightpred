...
static int pack_double(grib_accessor* a, const double* val, size_t *len)
{
  ...
  if (*val == GRIB_MISSING_DOUBLE) {
    ...
    directionIncrementDouble = *val * (double)angleDivisor / (double)angleMultiplier;

    /* TODO investigate bug fixed by the following line */
  sprintf(str,"------- dir=%g\n",directionIncrementDouble);

    if ((long)directionIncrementDouble ==  directionIncrementDouble ) {
