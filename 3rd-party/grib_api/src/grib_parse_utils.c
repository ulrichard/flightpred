			...
			} else if(uname[i] == ']')
			{
				...
				if(!a){
					if (!fail) {
						...
						grib_context_log(h->context, GRIB_LOG_WARNING,"grib_recompose_name: Problem to recompose filename with : %s, no accessor found", loc);
...
grib_rule* grib_parse_rules_file( grib_context* gc,const char* filename)
{
	...
	if (!f) {
      grib_context_log(gc,(GRIB_LOG_ERROR)|(GRIB_LOG_PERROR),"Cannot open file %s",filename);
      return NULL;
    }
	fclose( f );
