...
} grib_accessor_data_g2secondary_bitmap;

extern grib_accessor_class* grib_accessor_class_data_secondary_bitmap;

static grib_accessor_class _grib_accessor_class_data_g2secondary_bitmap = {
    &grib_accessor_class_data_secondary_bitmap,                      /* super                     */
    "data_g2secondary_bitmap",                      /* name                      */
    sizeof(grib_accessor_data_g2secondary_bitmap),  /* size                      */
    0,                           /* inited */
...
static long value_count(grib_accessor* a)
{
  ...
  if((err = grib_get_long_internal(a->parent->h,self->number_of_values,&len)) != GRIB_SUCCESS)
    return 0;
...
static int pack_double(grib_accessor* a, const double* val, size_t *len)
{
  ...
  primary_bitmap= grib_context_malloc_clear(a->parent->h->context,primary_len*sizeof(double));
  if(!primary_bitmap) return GRIB_OUT_OF_MEMORY;

  secondary_len = *len ;
  secondary_bitmap= grib_context_malloc_clear(a->parent->h->context,secondary_len*sizeof(double));
  if(!secondary_bitmap) {
