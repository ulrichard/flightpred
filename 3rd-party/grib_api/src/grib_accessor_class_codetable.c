...
static void init(grib_accessor* a, const long len, grib_arguments* params) {
  ...
  if (a->flags & GRIB_ACCESSOR_FLAG_TRANSIENT) {
    a->length = 0;
	if (a->vvalue != NULL)
		grib_context_free_persistent(a->parent->h->context, a->vvalue);
    a->vvalue = grib_context_malloc_clear_persistent(a->parent->h->context,sizeof(grib_virtual_value));
...
static int pack_string(grib_accessor* a, const char* buffer, size_t *len)
{
  ...
  for(i = 0 ; i < table->size; i++)
    if(table->entries[i].abbreviation)
      if(strcmp(table->entries[i].abbreviation,buffer) == 0)
...
{
  if (a->vvalue != NULL)
    grib_context_free_persistent(context, a->vvalue);
