...
int grib_g1_step_get_steps(grib_accessor* a,long* start,long* end) {
	...
	*end   = p2;
    ...
    else if (!strcmp(stepType,"precip")) {*start =0; *end = p1;}

	if(*start > *end) *end = *start;

	if (u2s[unit] == u2s[step_unit] || (*start==0 && *end==0) ) return 0;

	newstart = (*start) * u2s[unit];
	newend   = (*end)   * u2s[unit];

	if (newstart % u2s[step_unit] != 0 || newend % u2s[step_unit] != 0) {
/*
		*start = -1;
		*end   = -1;
		if (self->step_unit != NULL)
		...
		*/
		...
		err = grib_set_long_internal(a->parent->h,self->step_unit,unit);
		/*
		...
		*start = newstart/u2s[step_unit];
		*end   = newend/u2s[step_unit];
	}
...
int grib_g1_step_apply_units(long *start,long *end,long* step_unit,
    ...
    while (*unit != units_index[index] && index!=8)
      index++;
