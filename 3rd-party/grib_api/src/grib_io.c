...
static int read_the_rest(reader* r,size_t message_length,unsigned char* tmp, int already_read)
{
	...
	if((r->read(r->read_data,buffer+already_read,message_length-already_read,&err) != message_length-already_read) || err)
		return err;

	if(buffer[message_length-4] != '7' ||
...
#define CHECK_TMP_SIZE(a) if(sizeof(tmp)<(a)) { fprintf(stderr,"%s:%d sizeof(tmp)<%s %d<%d\n", __FILE__,__LINE__,#a,sizeof(tmp),a); return GRIB_INTERNAL_ARRAY_TOO_SMALL; }

static int read_GRIB(reader* r)
{
	unsigned char tmp[16368]; /* Should be enough */
	size_t length = 0;
	long edition = 0;
	int  err = 0;
	int i = 0 ,j;
...
static int stdio_read(void* data,void* buf,int len,int* err)
{
	FILE* f = (FILE*)data;
