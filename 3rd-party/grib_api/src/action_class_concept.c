...
grib_action* grib_action_create_concept( grib_context* context,
    ...
    const char* masterDir,const char* localDir,int flags,int nofail )
{
...
static grib_concept_value* get_concept(grib_handle* h,grib_action_concept* self)
{
  ...
  if (*local && grib_context_full_path(context,local,buf)!=NULL) {
    c=grib_parse_concept_file(context,buf);
    grib_context_log(h->context,GRIB_LOG_DEBUG,
                     "Loading concept %s from %s",((grib_action*)self)->name,buf);
  }

  if (grib_context_full_path(context,master,buf)==NULL) {
    grib_context_log(h->context,GRIB_LOG_ERROR,
                     "Unable to load %s from %s ",((grib_action*)self)->name,buf);
    return NULL;
  }
  ...
  if(c) {
    ...
    last->next=grib_parse_concept_file(context,buf);
  } else
    c=grib_parse_concept_file(context,buf);
...
int grib_concept_apply(grib_handle* h,grib_action* act,const char* name)
{
  ...
  while(c) {
    grib_concept_value_name *n = c->names;
    while(n) {
      ...
      if(strcmp("default",n->name) == 0) cdef=c;
