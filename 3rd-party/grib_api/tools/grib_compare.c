...
static int blacklisted(const char* name) {
  grib_string* b=blacklist;
  while (b) {
    if (!strcmp(name,b->string))
      return 1;
    b=b->next;
  }
  return 0;
...
/*  {id, args, help}, on, command_line, value*/
    ...
    {"T:","factor","Compare data values using factor multipied by the tolerance specified in options -P -R -A.\n",0,1,0},
    {"w:",0,0,0,1,0},
    {"f",0,0,0,1,0},
    {"F",0,0,1,0,0},
    {"q",0,0,1,0,0},
...
int grib_tool_init(grib_runtime_options* options) {
  ...
  if (grib_options_on("b:")) {
    grib_string *next=0;
    int i=0;
    blacklist=grib_context_malloc_clear(context,sizeof(grib_string));
    blacklist->string=grib_context_strdup(context,options->set_values[0].name);
    next=blacklist;
    for (i=1;i<options->set_values_count;i++) {
      next->next=grib_context_malloc_clear(context,sizeof(grib_string));
      next->next->string=grib_context_strdup(context,options->set_values[i].name);
      next=next->next;
    }
      ...
      if(err1 == GRIB_SUCCESS && err2 == GRIB_SUCCESS)
      {
        ...
        if (countdiff) {
          ...
          if(len1 == 1)
            ...
            printf("long [%s] %d out of %d different\n",
                   name,countdiff,len1);
        }
      }
    ...
    case GRIB_TYPE_DOUBLE:
      ...
      if(err1 == GRIB_SUCCESS && err2 == GRIB_SUCCESS)
      {
        ...
        if (countdiff) {
          ...
          save_error(c,name);
          if (len1>1) {
            printf("double [%s]: %d out of %d different, ",name,countdiff,len1);
...
static int compare_handles(grib_handle* h1,grib_handle* h2,grib_runtime_options* options)
{
  ...
  if (blacklist && ( !listFromCommandLine || headerMode )) {
    grib_string* nextb=blacklist;
    while (nextb) {
      grib_clear(h1,nextb->string);
      grib_clear(h2,nextb->string);
      nextb=nextb->next;
    }
  }
  ...
  if (headerMode) {
    ...
    grib_handle *h11, *h22;
    GRIB_CHECK(grib_get_message_headers(h1,&msg1,&size1),0);
    GRIB_CHECK(grib_get_message_headers(h2,&msg2,&size2),0);
  ...
  if ( listFromCommandLine && onlyListed ) {
    ...
    const void *msg1=NULL,*msg2=NULL;
    size_t size1=0,size2=0;
    GRIB_CHECK(grib_get_message(h1,&msg1,&size1),0);
    GRIB_CHECK(grib_get_message(h2,&msg2,&size2),0);
    if (size1==size2 && !memcmp(msg1,msg2,size1))
       return 0;
