  ...
  while(r)  {
    if(!r->condition) {
      ...
      while(e)  {
        ...
        GRIB_CHECK(grib_expression_set_value(NULL,e->value,&(options->set_values[set])),NULL);
        set++;
        e = e->next;
      }
    }
...
int grib_tool_new_handle_action(grib_runtime_options* options, grib_handle* h) {
  ...
  while(r) {
    if(r->condition)  {
      long ok = 0;
      GRIB_CHECK(grib_expression_evaluate_long(h,r->condition,&ok),NULL);
      if(ok) {
        grib_rule_entry* e = r->entries;
        while(e) {
  ...
  if(scale != 1 || offset != 0)
  {
    ...
    GRIB_CHECK(grib_get_double_array(h,"values",v,&size),0);
