...
static void statistical_process(grib_handle* h,const parameter* p,double min,double max)
{
	...
	CHECK(eq(h,"numberOfTimeRangeSpecificationsDescribingTheTimeIntervalsUsedToCalculateTheStatisticallyProcessedField",1));
	CHECK(eq(h,"totalNumberOfDataValuesMissingInStatisticalProcess",0));
	CHECK(eq(h,"typeOfTimeIncrementBetweenSuccessiveFieldsUsedInTheStatisticalProcessing",2));
...

	{
		...
		if (!is_lam) CHECK((get(h,"endStep") % 6) == 0);  /* Every six hours */
		CHECK(get(h,"endStep") <= 24*30);  
	}


	if(get(h,"indicatorOfUnitOfTimeForTimeRangeOverWhichStatisticalProcessingIsDone") == 11)
	{
		/* Six hourly is OK */
		CHECK(get(h,"lengthOfTheTimeRangeOverWhichStatisticalProcessingIsDone")*6 + get(h,"startStep") == get(h,"endStep"));
	}
	else
	{
		CHECK(eq(h,"indicatorOfUnitOfTimeForTimeRangeOverWhichStatisticalProcessingIsDone",1)); /* Hours */
		CHECK(get(h,"lengthOfTheTimeRangeOverWhichStatisticalProcessingIsDone") + get(h,"startStep") == get(h,"endStep"));
	}
...
static void six_hourly(grib_handle* h,const parameter* p,double min,double max)
{
	...
	if(get(h,"indicatorOfUnitOfTimeForTimeRangeOverWhichStatisticalProcessingIsDone") == 11)
		CHECK(eq(h,"lengthOfTheTimeRangeOverWhichStatisticalProcessingIsDone",1));
	else
		CHECK(eq(h,"lengthOfTheTimeRangeOverWhichStatisticalProcessingIsDone",6));
...
static void three_hourly(grib_handle* h,const parameter* p,double min,double max)
{
	...
	if(get(h,"indicatorOfUnitOfTimeForTimeRangeOverWhichStatisticalProcessingIsDone") == 11)
		...
		CHECK(eq(h,"lengthOfTheTimeRangeOverWhichStatisticalProcessingIsDone",3));
...
void latlon_grid(grib_handle* h)
{
	...
	CHECK(eq(h,"numberOfOctetsForOptionalListOfNumbersDefiningNumberOfPoints",0));
	CHECK(eq(h,"interpretationOfListOfNumbersDefiningNumberOfPoints",0));
...
void verify(grib_handle* h)
{
	...
	if (!is_lam) CHECK((get(h,"step") % 6) == 0);
